...
import { b as useSettingsStore, u as useAuthStore, e as useAppToast, d as useRoute, n as navigateTo, s as soundOptions, q as getApiErrorMessage, _ as _export_sfc } from "../server.mjs";
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`</div></div><div class="min-w-0 flex-1" data-v-cdf68cb8><div class="flex items-center gap-2" data-v-cdf68cb8><p class="truncate text-base font-bold text-sber-black" data-v-cdf68cb8>${ssrInterpolate(unref(authStore).user?.name)}</p>`);
      ...
      _push(`</div><p class="truncate text-sm text-sber-gray" data-v-cdf68cb8>${ssrInterpolate(unref(authStore).user?.email)}</p>`);
      ...
      ssrRenderTeleport(_push, (_push2) => {
        ...
        if (unref(premiumModal)) {
          ...
          _push2(`<p class="text-center text-xs text-sber-gray mt-3" data-v-cdf68cb8> Оплата через Робокассу. После оплаты нажмите «Я оплатил — активировать». </p><button class="btn-secondary mt-2 w-full" type="button" data-v-cdf68cb8>Закрыть</button></div>`);
        } else {
          _push2(`<!---->`);
        }
      }, "body", false, _parent);
      ...
      ssrRenderTeleport(_push, (_push2) => {
        if (unref(comingSoonVisible)) {
          _push2(`<div class="fixed top-20 left-1/2 -translate-x-1/2 bg-sber-black text-white px-5 py-3 rounded-2xl text-sm font-medium z-50 shadow-lg text-center" data-v-cdf68cb8> Уже разрабатываем, скоро будет готово 😊 </div>`);
        } else {
          _push2(`<!---->`);
        }
      }, "body", false, _parent);
...
//# sourceMappingURL=settings-CHxuNuC2.js.map

...
import { ref, computed, defineComponent, mergeProps, unref, createVNode, resolveDynamicComponent, useSSRContext } from "vue";
...
import { d as defineStore, a as useRoute, n as navigateTo } from "../server.mjs";
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const dayTimelineTasks = computed(() => {
      return tasksStore.getTasksForDate(calendarStore.currentDate).filter((t) => !!t.dueTime).map((task) => {
        ...
        const labelTime = preview ? `${formatMinutesToTime(preview.start)} – ${formatMinutesToTime(preview.end)}` : task.duration?.end ? `${task.dueTime} – ${task.duration.end}` : task.dueTime || "";
        return {
          ...task,
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(calendarStore).viewType === "day") {
        ...
        ssrRenderList(unref(dayTimelineTasks), (task) => {
          _push(`<div class="pointer-events-auto absolute left-1 right-1 cursor-grab touch-none select-none overflow-hidden rounded-xl px-3 py-2 transition-opacity active:opacity-70" style="${ssrRenderStyle({
            top: `${task.topPx}px`,
...
//# sourceMappingURL=calendar-BBKjOx3N.js.map

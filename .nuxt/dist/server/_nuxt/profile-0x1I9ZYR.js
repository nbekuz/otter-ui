...
import { ssrRenderAttrs, ssrRenderComponent, ssrRenderClass, ssrInterpolate, ssrRenderAttr, ssrRenderTeleport, ssrRenderList } from "vue/server-renderer";
import { ChevronLeft, Camera, Settings, ChevronRight, Crown } from "lucide-vue-next";
  ...
  setup(__props) {
    ...
    computed(() => [
      ...
      },
      {
        ...
        caption: authStore.user?.isPremium ? premiumExpiresLabel.value ? `До ${premiumExpiresLabel.value}` : "Премиум активен" : "Базовый тариф"
      }
    ]);
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(authStore).user?.isPremium && unref(premiumExpiresLabel)) {
        _push(`<div class="mt-2 flex flex-wrap items-center gap-2"><span class="rounded-full bg-yellow-100 px-2.5 py-1 text-[11px] font-bold text-yellow-700"> ⭐ ПРЕМИУМ </span><span class="text-xs font-medium text-yellow-700"> Срок до ${ssrInterpolate(unref(premiumExpiresLabel))}</span></div>`);
      } else {
        _push(`<!---->`);
      }
      ...
      ssrRenderTeleport(_push, (_push2) => {
        ...
        if (unref(avatarModal)) {
          ...
          ssrRenderList(unref(avatarOptions), (avatar) => {
            _push2(`<button class="${ssrRenderClass([unref(authStore).user?.avatar === avatar.url ? "border-sber-green" : "border-transparent", "overflow-hidden rounded-2xl border-2 bg-sber-gray-light p-1 transition-colors"])}" type="button"><img${ssrRenderAttr("src", avatar.url)}${ssrRenderAttr("alt", avatar.label)} class="h-24 w-full rounded-xl object-cover"></button>`);
          });
...
//# sourceMappingURL=profile-0x1I9ZYR.js.map

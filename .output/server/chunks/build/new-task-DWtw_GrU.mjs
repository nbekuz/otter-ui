...
import { defineComponent, ref, computed, reactive, watch, mergeProps, unref, createVNode, resolveDynamicComponent, nextTick, useSSRContext } from 'vue';
import { ssrRenderAttrs, ssrRenderComponent, ssrInterpolate, ssrRenderAttr, ssrRenderClass, ssrRenderList, ssrRenderVNode, ssrRenderStyle, ssrRenderTeleport } from 'vue/server-renderer';
import { ChevronLeft, Paperclip, X, Calendar, Flag, Bell, RefreshCw, Grid2x2, Check, Trash2 } from 'lucide-vue-next';
import dayjs from 'dayjs';
import { e as useRoute, d as useTasksStore, m as addMinutesToTime, n as navigateTo, r as resolveMediaUrl, q as matrixBlockDefaults } from './server.mjs';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const pendingLeavePath = ref(null);
    ...
    function readTimeQuery(key) {
      const raw = route.query[key];
      const value = Array.isArray(raw) ? raw[0] : raw;
      return typeof value === "string" && /^\d{2}:\d{2}$/.test(value) ? value : void 0;
    }
    const MATRIX_BLOCK_IDS = /* @__PURE__ */ new Set([
      "urgent-important",
      "not-urgent-important",
      "urgent-not-important",
      "not-urgent-not-important"
    ]);
    const PRIORITY_VALUES = /* @__PURE__ */ new Set(["high", "medium", "low", "none"]);
    function getMatrixBlockDefaultPriority(blockId) {
      var _a;
      const block = matrixBlockDefaults[blockId];
      const priority = (_a = block == null ? void 0 : block.priorityFilter) == null ? void 0 : _a[0];
      return priority && PRIORITY_VALUES.has(priority) ? priority : "none";
    }
    function applyPrefillFromQuery() {
      const matrixBlockParam = Array.isArray(route.query.matrixBlock) ? route.query.matrixBlock[0] : route.query.matrixBlock;
      if (typeof matrixBlockParam === "string" && MATRIX_BLOCK_IDS.has(matrixBlockParam)) {
        form.matrixBlock = matrixBlockParam;
        form.priority = getMatrixBlockDefaultPriority(matrixBlockParam);
        activeTab.value = "matrix";
      }
      const priorityParam = Array.isArray(route.query.priority) ? route.query.priority[0] : route.query.priority;
      if (typeof priorityParam === "string" && PRIORITY_VALUES.has(priorityParam)) {
        form.priority = priorityParam;
      }
      ...
      if (durationStartParam) {
        ...
        form.durationEnd = durationEndParam != null ? durationEndParam : addMinutesToTime(durationStartParam, 60);
    ...
    function hydrateFromTask(task) {
      ...
      if ((_j = task.attachment) == null ? void 0 : _j.dataUrl) {
    ...
    async function loadEditTaskFromRoute() {
      ...
      if (!task.attachment && !task.imageUrl) {
        try {
          ...
          const url = ((_a = detail.attachment) == null ? void 0 : _a.dataUrl) || detail.imageUrl;
          if (url && !attachmentDataUrl.value && !attachmentRemoved.value) {
            attachmentName.value = ((_b = detail.attachment) == null ? void 0 : _b.name) || "attachment";
            attachmentMimeType.value = ((_c = detail.attachment) == null ? void 0 : _c.mimeType) || "image/*";
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(attachmentName)) {
        ...
        if (unref(attachmentPreviewUrl)) {
          _push(`<a${ssrRenderAttr("href", unref(attachmentDataUrl))} target="_blank" rel="noopener"><img${ssrRenderAttr("src", unref(attachmentPreviewUrl))} alt="\u041F\u0440\u0435\u0434\u043F\u0440\u043E\u0441\u043C\u043E\u0442\u0440 \u0432\u043B\u043E\u0436\u0435\u043D\u0438\u044F" class="h-12 w-12 rounded-lg object-cover lg:h-16 lg:w-16 lg:rounded-xl"></a>`);
        } else {
          _push(`<!---->`);
        }
        _push(`<div class="min-w-0 flex-1">`);
        if (unref(attachmentDataUrl)) {
          _push(`<a${ssrRenderAttr("href", unref(attachmentDataUrl))} target="_blank" rel="noopener" class="truncate text-xs font-medium text-sber-green hover:underline lg:text-sm">${ssrInterpolate(unref(attachmentName))}</a>`);
        } else {
          _push(`<p class="truncate text-xs font-medium text-sber-black lg:text-sm">${ssrInterpolate(unref(attachmentName))}</p>`);
        }
        _push(`<p class="mt-0.5 text-[10px] text-sber-gray lg:mt-1 lg:text-xs">${ssrInterpolate(unref(attachmentPreviewUrl) ? "\u0418\u0437\u043E\u0431\u0440\u0430\u0436\u0435\u043D\u0438\u0435 \u043F\u0440\u0438\u043A\u0440\u0435\u043F\u043B\u0435\u043D\u043E" : "\u0424\u0430\u0439\u043B \u043F\u0440\u0438\u043A\u0440\u0435\u043F\u043B\u0435\u043D")}</p></div><button type="button" class="rounded-lg p-1 text-sber-gray transition-colors hover:bg-white hover:text-red-500">`);
      ...
      ssrRenderList(tabs, (tab) => {
        ...
        _push(`<span class="${ssrRenderClass(tab.iconOnly ? "max-lg:sr-only" : "")}">${ssrInterpolate(tab.label)}</span></button>`);
      });
      ...
      if (unref(activeTab) === "date") {
        ...
        if (!unref(explicitNoDeadline)) {
          _push(`<!--[--><div class="grid grid-cols-2 gap-x-2 gap-y-1.5 lg:gap-3"><div class="min-w-0"><p class="mb-0.5 text-[10px] font-semibold uppercase tracking-wide text-sber-gray lg:mb-2 lg:text-xs">\u0414\u0430\u0442\u0430</p>`);
          ...
          _push(`</div><div class="min-w-0"><p class="mb-0.5 text-[10px] font-semibold uppercase tracking-wide text-sber-gray lg:mb-2 lg:text-xs">\u0412\u0440\u0435\u043C\u044F \u0441\u0440\u043E\u043A\u0430</p>`);
          ...
          _push(`</div><div class="min-w-0"><p class="mb-0.5 text-[10px] font-semibold uppercase tracking-wide text-sber-gray lg:mb-2 lg:text-xs">\u041D\u0430\u0447\u0430\u043B\u043E</p>`);
          _push(ssrRenderComponent(_component_TimeFieldRu, {
          ...
          _push(`</div><div class="min-w-0"><p class="mb-0.5 text-[10px] font-semibold uppercase tracking-wide text-sber-gray lg:mb-2 lg:text-xs">\u041A\u043E\u043D\u0435\u0446</p>`);
          _push(ssrRenderComponent(_component_TimeFieldRu, {
      ...
      if (unref(activeTab) === "notify") {
        ...
        ssrRenderList(notifyOptions, (n) => {
          ...
          _push(`<span class="${ssrRenderClass([unref(form).notification === n.value ? "font-medium text-sber-green" : "text-sber-black", "text-xs lg:text-sm"])}">${ssrInterpolate(n.label)}</span>`);
        ...
        _push(`<!--]-->`);
        if (unref(form).notification === "custom") {
          _push(`<div class="mt-2 flex items-center gap-2 px-1"><input${ssrRenderAttr("value", unref(customNotifyMinutes))} type="number" min="0" max="10080" class="w-28 rounded-xl border border-sber-gray-mid bg-white px-3 py-2 text-sm font-semibold"><span class="text-xs text-sber-gray">\u043C\u0438\u043D\u0443\u0442 \u0434\u043E \u0441\u0440\u043E\u043A\u0430</span></div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div></div>`);
      ...
      if (unref(activeTab) === "repeat") {
        ...
        ssrRenderList(repeatOptions, (r) => {
          ...
          _push(`<span class="${ssrRenderClass([unref(form).repeat === r.value ? "font-medium text-sber-green" : "text-sber-black", "text-xs lg:text-sm"])}">${ssrInterpolate(r.label)}</span>`);
      ...
      if (unref(isEditMode) && unref(editingTask)) {
        ...
        _push(` ${ssrInterpolate(unref(editingTask).completed ? "\u0412\u043E\u0441\u0441\u0442\u0430\u043D\u043E\u0432\u0438\u0442\u044C" : "\u0412\u044B\u043F\u043E\u043B\u043D\u0435\u043D\u043E")}</button><div class="flex gap-1.5"><button class="btn-secondary min-w-0 flex-1 !w-auto !py-2.5 !text-xs" type="button"> \u041E\u0442\u043C\u0435\u043D\u0430 </button><button class="flex min-w-0 flex-[0.9] items-center justify-center gap-1 rounded-2xl bg-red-50 py-2.5 text-[11px] font-semibold text-red-500 transition-colors" type="button">`);
        _push(ssrRenderComponent(unref(Trash2), { class: "h-3.5 w-3.5 shrink-0" }, null, _parent));
        _push(`<span class="truncate">\u0423\u0434\u0430\u043B\u0438\u0442\u044C</span></button><button class="btn-primary min-w-0 flex-[1.15] !w-auto !py-2.5 !text-xs" type="submit"> \u0421\u043E\u0445\u0440\u0430\u043D\u0438\u0442\u044C </button></div></div><div class="hidden lg:block"><div class="mb-3 space-y-2"><button class="${ssrRenderClass([unref(editingTask).completed ? "bg-sber-gray-light text-sber-gray" : "bg-sber-green-light text-sber-green", "flex w-full items-center justify-center gap-2 rounded-2xl py-3.5 text-sm font-semibold transition-colors"])}" type="button">`);
        _push(ssrRenderComponent(unref(Check), { class: "h-5 w-5" }, null, _parent));
        _push(` ${ssrInterpolate(unref(editingTask).completed ? "\u0412\u043E\u0441\u0441\u0442\u0430\u043D\u043E\u0432\u0438\u0442\u044C" : "\u0412\u044B\u043F\u043E\u043B\u043D\u0435\u043D\u043E")}</button><button class="flex w-full items-center justify-center gap-2 rounded-2xl bg-red-50 py-3.5 text-sm font-semibold text-red-500 transition-colors" type="button">`);
        _push(ssrRenderComponent(unref(Trash2), { class: "h-5 w-5" }, null, _parent));
        _push(` \u0423\u0434\u0430\u043B\u0438\u0442\u044C </button></div><div class="flex gap-3"><button class="btn-secondary !w-auto flex-1" type="button"> \u041E\u0442\u043C\u0435\u043D\u0430 </button><button class="btn-primary !w-auto flex-1" type="submit"> \u0421\u043E\u0445\u0440\u0430\u043D\u0438\u0442\u044C </button></div></div><!--]-->`);
      } else {
        _push(`<!--[--><div class="flex gap-1.5 lg:hidden"><button class="btn-secondary min-w-0 flex-1 !w-auto !py-2.5 !text-sm" type="button"> \u041E\u0442\u043C\u0435\u043D\u0430 </button><button class="btn-primary min-w-0 flex-[1.35] !w-auto !py-2.5 !text-sm" type="submit"> \u0414\u043E\u0431\u0430\u0432\u0438\u0442\u044C \u0437\u0430\u0434\u0430\u0447\u0443 </button></div><div class="hidden lg:flex lg:gap-3"><button class="btn-secondary !w-auto flex-1" type="button"> \u041E\u0442\u043C\u0435\u043D\u0430 </button><button class="btn-primary !w-auto flex-1" type="submit"> \u0414\u043E\u0431\u0430\u0432\u0438\u0442\u044C \u0437\u0430\u0434\u0430\u0447\u0443 </button></div><!--]-->`);
      }
      _push(`</div></form></div>`);
...
//# sourceMappingURL=new-task-DWtw_GrU.mjs.map

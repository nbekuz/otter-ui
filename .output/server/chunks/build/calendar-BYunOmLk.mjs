import { _ as _sfc_main$1 } from './TaskDetailModal-D3aJuBg_.mjs';
...
import { c as useTasksStore, o as onClickOutside, g as getTaskScheduleStart, p as parseTimeToMinutes, h as formatMinutesToTime, f as defineStore } from './server.mjs';
import './useTaskTimeSync-CwnXrEZt.mjs';
      ...
      if (unref(calendarStore).viewType === "day") {
        ...
        if (!unref(calendarStore).collapsedEarlyHours) {
          ...
          ssrRenderList(unref(earlyTimelineTasks), (task) => {
            ...
            _push(`</button><p class="truncate text-xs font-medium text-sber-black">${ssrInterpolate(task.title)}</p></div></div>`);
          });
          _push(`<!--]--></div></div>`);
        } else {
          _push(`<!---->`);
        ...
        ssrRenderList(unref(dayTimelineTasks), (task) => {
          ...
          _push(`<div class="${ssrRenderClass([task.completed ? "opacity-45" : "", "pointer-events-auto absolute cursor-grab touch-none select-none overflow-hidden rounded-xl px-3 py-2 transition-opacity active:opacity-70"])}" style="${ssrRenderStyle({
            ...
            backgroundColor: getPriorityColor(task.priority) + "20",
            borderLeft: `3px solid ${getPriorityColor(task.priority)}`
          })}"><button type="button" class="absolute left-1/2 top-0 z-40 flex h-8 w-full max-w-[5.5rem] -translate-x-1/2 cursor-ns-resize items-start justify-center pt-1" aria-label="\u0418\u0437\u043C\u0435\u043D\u0438\u0442\u044C \u043D\u0430\u0447\u0430\u043B\u043E"><span class="pointer-events-none h-2 w-10 shrink-0 rounded-full bg-sber-gray/50"></span></button><div class="pointer-events-none relative z-10 flex items-start gap-1"><button type="button" class="pointer-events-auto mt-0.5 flex h-3.5 w-3.5 shrink-0 items-center justify-center rounded border" style="${ssrRenderStyle({ borderColor: getPriorityColor(task.priority), backgroundColor: task.completed ? getPriorityColor(task.priority) : "transparent" })}">`);
          if (task.completed) {
            _push(ssrRenderComponent(unref(Check), { class: "h-2 w-2 text-white" }, null, _parent));
          } else {
            _push(`<!---->`);
          }
          _push(`</button><div class="min-w-0 flex-1"><p class="text-xs font-semibold" style="${ssrRenderStyle({ color: getPriorityColor(task.priority) })}">${ssrInterpolate(task.labelTime)}</p><p class="truncate text-xs font-medium text-sber-black">${ssrInterpolate(task.title)}</p></div></div><button type="button" class="absolute bottom-0 left-1/2 z-40 flex h-8 w-full max-w-[5.5rem] -translate-x-1/2 cursor-ns-resize items-end justify-center pb-1" aria-label="\u0418\u0437\u043C\u0435\u043D\u0438\u0442\u044C \u043A\u043E\u043D\u0435\u0446"><span class="pointer-events-none h-2 w-10 shrink-0 rounded-full bg-sber-gray/50"></span></button></div>`);
        });
...
//# sourceMappingURL=calendar-BYunOmLk.mjs.map

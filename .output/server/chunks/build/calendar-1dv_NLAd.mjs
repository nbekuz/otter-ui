import { _ as _sfc_main$1 } from './TaskDetailModal-Bbs2rPRZ.mjs';
...
import { d as useTasksStore, o as onClickOutside, j as getTaskScheduleStart, p as parseTimeToMinutes, k as getTaskDurationMinutes, l as formatMinutesToTime, i as defineStore } from './server.mjs';
import './useTaskTimeSync-BMCBFEST.mjs';
import 'dayjs/plugin/customParseFormat.js';
import './priority-colors-BPjPHsbX.mjs';
...
function assignTimelineOverlapLayout(segments) {
...
function timelineTaskHorizontalStyle(layoutCols, layoutCol, pad = 4, gap = 3) {
  if (layoutCols <= 1) {
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    ref(null);
    const untimedHeightPx = ref(UNTIMED_ROW_PX * 3);
    const viewTypes = [
    ...
    function weekMinutesToPx(min) {
      ...
      return idx * weekHourHeightPx + (min - hour * 60) * (weekHourHeightPx / 60);
    }
    ...
    function dayTimelineTaskHorizontalStyle(layoutCols, layoutCol) {
      return timelineTaskHorizontalStyle(layoutCols, layoutCol);
    }
    const dayTimelineTasks = computed(() => {
      const base = tasksStore.getTasksForDate(calendarStore.currentDate).filter((t) => !!getTaskScheduleStart(t)).map((task) => {
        ...
        const labelTime = preview ? `${formatMinutesToTime(preview.start)} \u2013 ${formatMinutesToTime(preview.end)}` : ((_b = task.duration) == null ? void 0 : _b.start) && ((_c = task.duration) == null ? void 0 : _c.end) ? `${task.duration.start} \u2013 ${task.duration.end}` : getTaskScheduleStart(task) || "";
        return {
          ...task,
    ...
    function getWeekDayTimelineTasks(date) {
      const base = tasksStore.getTasksForDate(date).filter((t) => !!getTaskScheduleStart(t)).map((task) => {
        var _a;
        const preview = ((_a = dragPreview.value) == null ? void 0 : _a.taskId) === task.id ? dragPreview.value : null;
        const scheduleStart = getTaskScheduleStart(task) || "00:00";
        const startMinutes = preview ? preview.start : parseTimeToMinutes(scheduleStart);
    ...
    function getUntimedTasksForDate(date) {
      return tasksStore.getTasksForDate(date).filter((t) => !getTaskScheduleStart(t) && !!t.dueDate);
    }
    ...
    watch(
      ...
      (tasks) => {
        ...
        const hasLate = tasks.some((task) => {
          ...
          const endHour = ((_a = task.duration) == null ? void 0 : _a.end) ? parseInt(task.duration.end.split(":")[0], 10) : hour;
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(calendarStore).viewType === "day") {
        ...
        if (unref(dayUntimedTasks).length) {
          _push(`<div class="sticky top-0 z-30 border-b border-sber-gray-mid bg-white shadow-sm"><div class="flex items-center justify-between px-3 pt-2"><p class="text-[10px] font-semibold uppercase tracking-wide text-sber-gray">\u0411\u0435\u0437 \u0432\u0440\u0435\u043C\u0435\u043D\u0438</p><span class="text-[10px] font-medium text-sber-gray">${ssrInterpolate(unref(dayUntimedTasks).length)}</span></div><div class="overflow-y-auto px-3 py-1.5" style="${ssrRenderStyle({ height: `${unref(untimedHeightPx)}px` })}"><div class="flex flex-col gap-1.5"><!--[-->`);
          ssrRenderList(unref(dayUntimedTasks), (task) => {
            _push(`<div class="${ssrRenderClass([task.completed ? "opacity-45" : "", "flex min-w-0 items-center gap-1.5 rounded-xl border px-2 py-1.5 text-xs"])}" style="${ssrRenderStyle({
          ...
          _push(`<!--]--></div></div><div class="flex cursor-ns-resize touch-none items-center justify-center py-1 transition-colors hover:bg-sber-gray-light" title="\u041F\u043E\u0442\u044F\u043D\u0438\u0442\u0435, \u0447\u0442\u043E\u0431\u044B \u0438\u0437\u043C\u0435\u043D\u0438\u0442\u044C \u0432\u044B\u0441\u043E\u0442\u0443"><span class="pointer-events-none h-1 w-10 rounded-full bg-sber-gray-mid"></span></div></div>`);
        } else {
          _push(`<!---->`);
        }
        ...
        if (!unref(calendarStore).collapsedEarlyHours) {
          ...
          ssrRenderList(earlyHours, (h) => {
            _push(`<div class="flex min-h-[60px]"><div class="w-14 flex-shrink-0 border-t border-sber-gray-mid text-xs text-sber-gray text-right pr-3 pt-1">${ssrInterpolate(String(h).padStart(2, "0"))}:00 </div><div class="flex-1 cursor-pointer border-t border-sber-gray-mid"></div></div>`);
          });
          _push(`<!--]--><div class="pointer-events-none absolute top-0 right-0 bottom-0 left-14"><!--[-->`);
          ssrRenderList(unref(earlyTimelineTasks), (task) => {
            var _a;
            _push(`<div class="${ssrRenderClass([task.completed ? "opacity-45" : "", "pointer-events-auto absolute cursor-grab touch-none select-none overflow-hidden rounded-xl px-2 py-1"])}" style="${ssrRenderStyle({
            ...
            })}"><button type="button" class="absolute left-1/2 top-0 z-40 flex h-6 w-full max-w-[5.5rem] -translate-x-1/2 cursor-ns-resize items-start justify-center pt-0.5" aria-label="\u0418\u0437\u043C\u0435\u043D\u0438\u0442\u044C \u043D\u0430\u0447\u0430\u043B\u043E"><span class="pointer-events-none h-1.5 w-8 shrink-0 rounded-full bg-sber-gray/50"></span></button><div class="flex min-w-0 items-start gap-1"><button type="button" class="mt-0.5 flex h-3.5 w-3.5 shrink-0 items-center justify-center rounded border" style="${ssrRenderStyle({ borderColor: getPriorityColor(task.priority), backgroundColor: task.completed ? getPriorityColor(task.priority) : "transparent" })}">`);
            if (task.completed) {
              _push(ssrRenderComponent(unref(Check), { class: "h-2 w-2 text-white" }, null, _parent));
            } else {
              _push(`<!---->`);
            }
            _push(`</button><div class="min-w-0 flex-1 overflow-hidden"><p class="truncate text-xs font-medium text-sber-black">${ssrInterpolate(task.title)}</p>`);
            if (task.continuesAfter) {
              _push(`<p class="text-[10px] text-sber-gray">\u043F\u0440\u043E\u0434\u043E\u043B\u0436\u0430\u0435\u0442\u0441\u044F \u2193</p>`);
            } else {
              _push(`<!---->`);
            }
            _push(`</div></div><button type="button" class="absolute bottom-0 left-1/2 z-40 flex h-6 w-full max-w-[5.5rem] -translate-x-1/2 cursor-ns-resize items-end justify-center pb-0.5" aria-label="\u0418\u0437\u043C\u0435\u043D\u0438\u0442\u044C \u043A\u043E\u043D\u0435\u0446"><span class="pointer-events-none h-1.5 w-8 shrink-0 rounded-full bg-sber-gray/50"></span></button></div>`);
          });
          _push(`<!--]--></div></div>`);
        } else {
          _push(`<!---->`);
        ...
        if (!unref(calendarStore).collapsedLateHours) {
          ...
          ssrRenderList(lateHours, (h) => {
            _push(`<div class="flex min-h-[60px]"><div class="w-14 flex-shrink-0 border-t border-sber-gray-mid text-xs text-sber-gray text-right pr-3 pt-1">${ssrInterpolate(String(h).padStart(2, "0"))}:00 </div><div class="flex-1 cursor-pointer border-t border-sber-gray-mid px-1"></div></div>`);
          });
          _push(`<!--]--><div class="border-t border-sber-gray-mid"></div><div class="pointer-events-none absolute top-0 right-0 bottom-0 left-14"><!--[-->`);
          ssrRenderList(unref(lateTimelineTasks), (task) => {
            ...
            _push(`</button><div class="min-w-0 flex-1 overflow-hidden"><p class="truncate text-xs font-semibold" style="${ssrRenderStyle({ color: getPriorityColor(task.priority) })}">${ssrInterpolate(formatTaskScheduleLabel(task))}</p><p class="truncate text-xs font-medium text-sber-black">${ssrInterpolate(task.title)}</p></div></div><button type="button" class="absolute bottom-0 left-1/2 z-40 flex h-6 w-full max-w-[5.5rem] -translate-x-1/2 cursor-ns-resize items-end justify-center pb-0.5" aria-label="\u0418\u0437\u043C\u0435\u043D\u0438\u0442\u044C \u043A\u043E\u043D\u0435\u0446"><span class="pointer-events-none h-1.5 w-8 shrink-0 rounded-full bg-sber-gray/50"></span></button></div>`);
          });
          _push(`<!--]--></div></div>`);
        } else {
          _push(`<!---->`);
        ...
        _push(`<div class="relative p-2"><div class="sticky top-0 z-30 bg-sber-gray-light/95 pb-1 backdrop-blur-sm"><div class="mb-1 flex gap-1"><div class="w-14 flex-shrink-0"></div><!--[-->`);
        ssrRenderList(unref(weekViewDays), (day) => {
          _push(`<div class="min-w-0 flex-1 cursor-pointer text-center"><p class="text-xs text-sber-gray">${ssrInterpolate(day.dayName)}</p><div class="${ssrRenderClass([day.isToday ? "bg-sber-green text-white" : "text-sber-black", "mx-auto flex h-8 w-8 items-center justify-center rounded-full"])}"><span class="text-sm font-bold">${ssrInterpolate(day.dayNum)}</span></div></div>`);
        });
        _push(`<!--]--></div><div class="flex gap-1"><div class="w-14 flex-shrink-0 pt-1 text-right text-[10px] font-semibold uppercase text-sber-gray"> \u0411\u0435\u0437 \u0432\u0440. </div><!--[-->`);
        ssrRenderList(unref(weekViewDays), (day) => {
          _push(`<div class="min-w-0 flex-1 overflow-y-auto border-b border-[#c8cfdb] px-0.5 py-0.5" style="${ssrRenderStyle({ height: `${unref(untimedHeightPx)}px` })}"><div class="flex flex-col gap-0.5"><!--[-->`);
          ssrRenderList(getUntimedTasksForDate(day.date), (task) => {
            _push(`<div class="${ssrRenderClass([task.completed ? "opacity-45" : "", "flex min-w-0 items-center gap-0.5 rounded border px-1 py-0.5 text-xs"])}" style="${ssrRenderStyle({ backgroundColor: getPriorityColor(task.priority) + "22", borderColor: getPriorityColor(task.priority) + "40" })}" draggable="true"><button type="button" class="flex h-2.5 w-2.5 shrink-0 items-center justify-center rounded border" style="${ssrRenderStyle({ borderColor: getPriorityColor(task.priority), backgroundColor: task.completed ? getPriorityColor(task.priority) : "transparent" })}">`);
            if (task.completed) {
        ...
        _push(`<!--]--></div><div class="mb-1 flex cursor-ns-resize touch-none items-center justify-center py-0.5 transition-colors hover:bg-sber-gray-light" title="\u041F\u043E\u0442\u044F\u043D\u0438\u0442\u0435, \u0447\u0442\u043E\u0431\u044B \u0438\u0437\u043C\u0435\u043D\u0438\u0442\u044C \u0432\u044B\u0441\u043E\u0442\u0443"><span class="pointer-events-none h-1 w-10 rounded-full bg-sber-gray-mid"></span></div></div><div class="overflow-hidden rounded-2xl border border-[#c8cfdb] bg-white/45"><div class="flex cursor-pointer items-center bg-sber-gray-light px-2 py-1.5"><div class="w-12 flex-shrink-0 pr-2 text-right text-[10px] text-sber-gray">00\u201306</div><div class="h-px flex-1 bg-sber-gray-mid"></div>`);
        ssrRenderVNode(_push, createVNode(resolveDynamicComponent(unref(calendarStore).collapsedEarlyHours ? unref(ChevronDown) : unref(ChevronUp)), { class: "ml-2 h-4 w-4 text-sber-gray" }, null), _parent);
        _push(`</div><div class="flex"><div class="sticky left-0 z-10 w-14 flex-shrink-0 bg-white/90"><!--[-->`);
        ssrRenderList(unref(visibleWeekHours), (h) => {
          _push(`<div class="border-t border-[#c8cfdb] pr-2 pt-1 text-right text-xs text-sber-gray first:border-t-0" style="${ssrRenderStyle({ height: `${weekHourHeightPx}px` })}">${ssrInterpolate(String(h).padStart(2, "0"))}:00 </div>`);
        });
        _push(`<!--]--></div><!--[-->`);
        ssrRenderList(unref(weekViewDays), (day) => {
          _push(`<div class="relative min-w-0 flex-1 border-l border-[#c8cfdb]"><!--[-->`);
          ssrRenderList(unref(visibleWeekHours), (h) => {
            _push(`<div class="cursor-pointer border-t border-[#c8cfdb] first:border-t-0" style="${ssrRenderStyle({ height: `${weekHourHeightPx}px` })}"></div>`);
          });
          _push(`<!--]--><!--[-->`);
          ssrRenderList(getWeekDayTimelineTasks(day.date), (task) => {
            _push(`<div class="${ssrRenderClass([task.completed ? "opacity-45" : "", "absolute cursor-move overflow-hidden rounded border border-[#c8cfdb] px-1 py-0.5"])}" style="${ssrRenderStyle({
              ...dayTimelineTaskHorizontalStyle(task.layoutCols, task.layoutCol),
              top: `${task.topPx}px`,
              height: `${task.heightPx}px`,
              backgroundColor: getPriorityColor(task.priority) + "22"
            })}" draggable="true"><button type="button" class="absolute left-1/2 top-0 z-40 flex h-5 w-full -translate-x-1/2 cursor-ns-resize items-start justify-center" aria-label="\u0418\u0437\u043C\u0435\u043D\u0438\u0442\u044C \u043D\u0430\u0447\u0430\u043B\u043E"><span class="pointer-events-none h-1 w-6 shrink-0 rounded-full bg-sber-gray/50"></span></button><div class="flex min-w-0 items-start gap-0.5"><button type="button" class="mt-px flex h-2.5 w-2.5 shrink-0 items-center justify-center rounded border" style="${ssrRenderStyle({ borderColor: getPriorityColor(task.priority), backgroundColor: task.completed ? getPriorityColor(task.priority) : "transparent" })}">`);
            if (task.completed) {
              _push(ssrRenderComponent(unref(Check), { class: "h-1.5 w-1.5 text-white" }, null, _parent));
            } else {
              _push(`<!---->`);
            }
            _push(`</button><div class="min-w-0 flex-1 overflow-hidden"><p class="truncate text-xs font-semibold text-sber-black">${ssrInterpolate(formatTaskScheduleLabel(task))}</p><p class="truncate text-xs font-medium text-sber-black">${ssrInterpolate(task.title)}</p></div></div><button type="button" class="absolute bottom-0 left-1/2 z-40 flex h-5 w-full -translate-x-1/2 cursor-ns-resize items-end justify-center" aria-label="\u0418\u0437\u043C\u0435\u043D\u0438\u0442\u044C \u043A\u043E\u043D\u0435\u0446"><span class="pointer-events-none h-1 w-6 shrink-0 rounded-full bg-sber-gray/50"></span></button></div>`);
          });
          _push(`<!--]--></div>`);
        });
        _push(`<!--]--></div><div class="flex cursor-pointer items-center bg-sber-gray-light px-2 py-1.5"><div class="w-12 flex-shrink-0 pr-2 text-right text-[10px] text-sber-gray">22\u201300</div><div class="h-px flex-1 bg-sber-gray-mid"></div>`);
        ssrRenderVNode(_push, createVNode(resolveDynamicComponent(unref(calendarStore).collapsedLateHours ? unref(ChevronDown) : unref(ChevronUp)), { class: "ml-2 h-4 w-4 text-sber-gray" }, null), _parent);
        _push(`</div></div></div>`);
...
//# sourceMappingURL=calendar-1dv_NLAd.mjs.map
